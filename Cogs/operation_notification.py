...
NOTIFICATION_OPTIONS = {
    ...
    "30MIN_OPS": OperationMessageOptions(
        show_game=False, show_date_end=False, include_timestamp=False
    ),
}
...
class OperationsEmbed:
    ...
    def __init__(
        self, title: str, notification_options: OperationMessageOptions
    ) -> None:
        """Create the embed to be used by operation messages"""
        self.embed = discord.Embed(
            title=title, color=notification_options.color, type="rich"
        )
    ...
    async def send_operations(
        ...
        if self.options.include_timestamp:
            ...
            self.embed.set_footer(
                text=f"Last updated: {timestamp.strftime('%Y-%m-%d %H:%M')}"
            )
    ...
    def get_operations(
        ...
        return (
            ...
            .where(
                ...
                operation_model.operation_id not in exclude,
            )
    ...
    async def send(self) -> [database.Operation]:
        ...
        for game, data in self.config.opsec_channels_map.items():
            for access, channels in data.items():
                ...
                operations = await super().send_operations(
                    "Operations starting in 30 minutes!",
                    channels=channels,
        ...
        if notifications_sent:
            ...
            for op in operations:
                op_data.append(
                    {"operation_id": op.operation_id, "date_start": op.date_start}
                )
...
class NotificationTask:
    ...
    def __init__(
        self, game_id: int, is_opsec: int, channel: int, task: asyncio.Task
    ) -> None:
...
class UpcomingOperationsNotifier(commands.Cog, OperationNotifier):
    ...
    def create_task(
        self, game: int, is_opsec: int, channel: int, cron: str
    ) -> NotificationTask:
    ...
    async def __send(
        self, game: int, is_opsec: int, schedule: str, channels: [int]
    ) -> None:
        ...
        while True:
            ...
            await super().send_operations(
                f"{title} Operations", channels=channels, operations=ops
            )
